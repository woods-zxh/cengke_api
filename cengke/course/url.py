...
urlpatterns = [
    ...
    path('course_table/<str:token>/', views.CourseTableView.as_view(), name='course_table'),  #用于返回用户的课程表
    ...
    path('course_history/<str:token>/', views.CourseHistoryView.as_view(), name='course_history'),  # 用于返回
    path('recommend/<str:token>/', views.RecommmentView.as_view(),name ='recommend'),#用于给用户推荐他的个性课程
    #蹭课足迹的获取
    #
